    ...
    while time.time() - start_time < (
        ...
        if rt == "PAGO":
            ...
            await c.send_message(
                ADMIN_CHAT,
                f"❇️Nova Recarga Realizada\n\n[Recarregar Agora!!](https://t.me/hydra7storebot)\n\n🙎‍♂️Cliente:{mention}\n💰Adicionou <b>R${value}</b> de saldo"
...
async def auto_pay(c: Client, m: Union[CallbackQuery, Message]):
    ...
    if isinstance(m, CallbackQuery):
        ...
        cob = await m.message.ask(
            f"""<b>Digite o valor que você quer depositar (minimo R$ {min_value}), exemplo:
25 
30
31 
Etc

Caso nao consiga efetuar o pagamento, chame o  {get_support_user()}</b>""",
        ...
        if len(m.command) == 1:
            cob = await m.ask(
                f"""<b>Digite o valor que voce quer depositar (mÃ­nimo R$ {min_value}), exemplo:
25 
30
31 
Etc
...
Se nao conseguir fazer pix automatico, chame o {get_support_user()}
"""
